...
enum VARIABLE_TYPES {SCALAR, TENSOR, INDEX, FUNCTION, LIMIT, SERIES, EXPRESSION};
enum VARIABLE_PROPERTIES {SYMMETRIC, ANTISYMMETRIC, NILL, DEPENDANT, CONSTANT, LINEAR, QUADRATIC, MONOTONE, INCREASING, DECREASING};
enum PROCESS_TYPE {RECCURSIVE, FIRST_LEVEL, DEFAULT};
...
class HSym {
    private:
        ...
        std::pair < std::vector< std::pair< std::vector< std::pair<HSym, HSym> >, HSym > >, HSym > * sumprodpow_matrix;
        /*\sum(\prod(a^p)^p)^p*/
        std::pair < std::vector< std::pair< std::vector< std::pair<HSym, HSym> >, HSym > >, HSym > * original_sumprodpow_matrix;
