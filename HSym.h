...
class HSymbolic{
    ...
    public: 
        ...
        HSymbolic _inv(){
            ...
            if(_x.ismultiply_ring()){
                auto _xstrip = _x;
                if(_x.func == NO_FUNC || _x.strip().value != NO_VAL)
                _xstrip = _x.strip();
                else _xstrip = _x;
               ...
               // std::cout<<"c"<<(_xstrip.ring_elements.begin()->second);
                if(_xstrip.ring_elements.begin()->second == 1 && _xstrip.ring_elements.size() == 1)
                    return _xstrip.strip();
                return _xstrip.hug();
